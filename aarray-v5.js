...
function AArray(data) {
	...
	this.extend = function(data) {
		if (data instanceof Array) for(var i=0; i < data.length; i++) this[i] = data[i];
		else if (data && data instanceof Object) for(i in data) this[i] = data[i];
		return this;
	};
	...
	Object.observe(this, function(changes) {
		for(var i=0; i < changes.length; i++) {
			switch (changes[i].type) {
				case 'add':
				case 'update':
					_this.set(changes[i].name, changes[i].object[changes[i].name]);
					break;
				case 'delete':
					_this.remove(changes[i].name);
					break;
			}
		}
		changes.shift();
	...
	for(var i in this) {
		if (this[i] instanceof Function) Object.defineProperty(this, i, { enumerable: false });
	}
	Object.defineProperty(this, 'keySet', { enumerable: false });
	Object.defineProperty(this, 'valueSet', { enumerable: false });
