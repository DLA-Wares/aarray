...
module.exports = function(data) {
	return new Proxy(new AArray(data), {
		...
		get: function(target, name, receiver) {
			...
			var i = target.keySet.indexOf(name);
			if (i > -1) return target.valueSet[i];
			else return target[name];
		},
		set: function(target, name, value, receiver) {
			var i = target.keySet.indexOf(name);
			if (i > -1) {
				target.valueSet[i] = value;
			}
			else {
				var i = target.keySet.push(name)-1;
				target.valueSet[i] = value;
			}
			// define property
			target[name] = null; // Platzhalter damit man mit Objekte wie gewohnt arbeiten kann ;-)
			target.length = target.keySet.length;
			return true;
		}
	});
